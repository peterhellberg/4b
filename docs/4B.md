...
console. A `.4b` source file is a sequence of instructions that map 1:1 onto
the 4BoD instruction set, plus a few conveniences (named constants, named
labels) that the 4B compiler (`4bc`) resolves away at build time. Every 4BoD
machine instruction is expressible in 4B, and every 4B source file compiles to
...
- The compiler assigns labels to slots `0..14` in order of first definition.
  **Slot 15 is reserved**: on the hardware it is hard-wired to `0` (a bug), so
  jumps to it land on instruction 0. `4bc` refuses to use slot 15.
...
The compiled image is **exactly 384 bytes** (256 × 12 bits = 3072 bits), no
header, no footer.
...
sta r2          ; x = x + 1
ifeq r3         ; wrapped to 0? then skip the jump
jmp @draw
...
The compiler assigns `@draw` → slot 0 and `@halt` → slot 1. The 15 emitted
words are `0x380 0x210 0x300 0x220 0x230 0xB00 0x120 0x200 0xA01 0x500 0x220
0xD30 0xC00 0xB10 0xC10`, packing to:

```
80 03 21 00 03 22 30 02 B0 20 01 20 01 0A 50 20 02 D3 00 0C B1 10 0C
```

…followed by 0x00 padding to byte 383. (Verify this with `4bc`, then run it
...
## 12. Limits and compile-time errors
...
Compile-time errors: undefined label, label redefined, too many labels, use of
...
- Source files: `.4b`
- Compiled images: `.4brom` (raw 384-byte ROM; loaders may name it anything)
- Compiler: `4bc` (see `docs/4bc.md`)
