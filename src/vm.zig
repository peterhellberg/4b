...
pub export fn fourb_vm_init(vm: *VM) void {
    @memset(&vm.program, 0);
    @memset(&vm.regs, 0);
    vm.acc = 0;
    @memset(&vm.screen, 0);
...
pub export fn fourb_vm_load_rom(vm: *VM, rom: [*]const u8, len: usize) void {
    fourb_vm_init(vm);
    var i: usize = 0;
...
/// Build one 12-bit instruction word from its opcode and operand nibbles.
fn inst(op: u4, a: u4, b: u4) u16 {
...
test "vm: flag records own position and jmp returns to it" {
    var vm: VM = undefined;
    fourb_vm_init(&vm);
    vm.program[13] = inst(0xB, 1, 0);
    vm.program[14] = inst(0xC, 1, 0);
    vm.pc = 13;
...
fn expectSkip(comptime op: u4, r: u4, acc: u4, executes_next: bool) !void {
    var vm: VM = undefined;
    fourb_vm_init(&vm);
    vm.program[100] = inst(op, 9, 0);
    vm.program[101] = inst(0x3, 15, 0);
    vm.pc = 100;
    vm.acc = acc;
    vm.regs[9] = r;
    fourb_vm_tick(&vm);
