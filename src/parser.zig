...
pub fn parse(alloc: std.mem.Allocator, diag: *diag_mod.Diag, tokens: []const Token) ParseError!std.ArrayList(Item) {
...
fn parseLine(alloc: std.mem.Allocator, diag: *diag_mod.Diag, tokens: []const Token, idx: *usize, items: *std.ArrayList(Item)) ParseError!void {
    while (true) {
...
fn parseStatement(alloc: std.mem.Allocator, diag: *diag_mod.Diag, tokens: []const Token, idx: *usize, items: *std.ArrayList(Item)) ParseError!void {
    const tok = tokens[idx.*];
    ...
    try items.append(alloc, .{ .inst = .{ .spec = spec, .a = a, .b = b, .line = tok.line, .col = tok.col } });
}

fn parseOperand(tokens: []const Token, idx: *usize, kind: model.OperandKind, diag: *diag_mod.Diag) ?Operand {
...
fn expectToken(tokens: []const Token, idx: *usize, comptime expected: Kind, diag: *diag_mod.Diag, comptime msg: []const u8) ?Token {
    const tok = peek(tokens, idx.*);
...
fn skipToEol(tokens: []const Token, idx: *usize) void {
    while (idx.* < tokens.len and tokens[idx.*].kind != .eol and tokens[idx.*].kind != .eof) {
        idx.* += 1;
    }
}
...
test "parse lda #8 sta r1" {
    ...
    defer arena.deinit();
    var diag = diag_mod.Diag.init(arena.allocator(), "<test>", "lda #8\nsta r1\n");
...
test "parse label definition" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var diag = diag_mod.Diag.init(arena.allocator(), "<test>", "@foo:\n");
...
test "parse jmp @label" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var diag = diag_mod.Diag.init(arena.allocator(), "<test>", "jmp @start\n");
...
test "parse const" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var diag = diag_mod.Diag.init(arena.allocator(), "<test>", "const X = 5\n");
...
test "parse peek r0, r1" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var diag = diag_mod.Diag.init(arena.allocator(), "<test>", "peek r0, r1\n");
