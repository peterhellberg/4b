...
test "bc_assemble assembles valid source and reports errors" {
    var out: [384]u8 = undefined;

    try std.testing.expectEqual(@as(c_int, 0), bc_assemble("t.4a", "jmp @h\n@h:\nnop\n", 15, &out, null, 0));
    try std.testing.expectEqual(@as(u8, 0x00), out[0]);
    try std.testing.expectEqual(@as(u8, 0x0C), out[1]);

    var err_buf: [256]u8 = undefined;

    try std.testing.expectEqual(@as(c_int, 1), bc_assemble("t.4a", "jmp @nowhere\n", 13, &out, &err_buf, err_buf.len));
